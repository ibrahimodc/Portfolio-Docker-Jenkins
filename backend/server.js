...
// POST /api/projets  — créer un projet (JSON ou multipart/form-data)
app.post("/api/projets", upload.single("image"), (req, res) => {
  const { libelle, description, technologies, lien, imageUrl } = req.body;
  if (!libelle)
    return res
      .status(400)
      .json({ success: false, message: "Le libellé est obligatoire." });
